...
const privacy = layout({
    ...
    body: `
      ...
      <p>Report on a post or profile sends the target canister, post or user ID, your reason, and a random request ID to the operator's Cloudflare Worker. Reports are stored in D1 for manual review; no token or credit balance is required. Reports do not automatically hide content. Do not include passwords, private keys, or other sensitive information. Describe the problem only; do not include illegal content itself, links to it, or other people's personal details.</p>
