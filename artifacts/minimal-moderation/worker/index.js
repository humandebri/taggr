...
// src/moderation.mjs
var CANISTERS = ["6qfxa-ryaaa-aaaai-qbhsq-cai"];
var validID = /* @__PURE__ */ __name((id) => Number.isSafeInteger(id) && id >= 0, "validID");
var uuid = /* @__PURE__ */ __name((id) => typeof id === "string" && /^[\da-f]{8}-[\da-f]{4}-4[\da-f]{3}-[89ab][\da-f]{3}-[\da-f]{12}$/i.test(
  id
), "uuid");
...
function validateReport(body) {
  if (!body || !uuid(body.id) || !CANISTERS.includes(body.canisterID) || !validID(body.userID) || body.postID != null && !validID(body.postID))
...
__name(readReport, "readReport");
async function moderationFetch(request, env) {
  const url = new URL(request.url);
  if (!["/api/reports", "/api/moderation"].includes(url.pathname))
    return null;
  try {
    if (url.pathname === "/api/reports" && request.method !== "POST" || url.pathname === "/api/moderation" && request.method !== "GET")
    ...
    if (url.pathname === "/api/moderation") {
      ...
      return json({
        ...
        postIDs: targets.results.filter((r) => r.kind === "post").map((r) => r.target_id),
        userIDs: targets.results.filter((r) => r.kind === "user").map((r) => r.target_id)
      });
    }
    ...
    const { success } = await env.REPORT_LIMITER.limit({
      key: "taggr-ios-reports:" + (request.headers.get("cf-connecting-ip") || "local")
    });
    if (!success)
      return json(
    ...
    if (!saved || saved.canister_id !== report.canisterID || saved.user_id !== report.userID || saved.post_id !== report.postID || saved.reason !== report.reason)
    ...
    return json(
      {
        error: error.status ? error.message : "Service temporarily unavailable. Please retry."
      },
      error.status || 503
    );
  }
}
__name(moderationFetch, "moderationFetch");
...
var privacy = layout({
  ...
  body: `
      ...
      <p>This policy may be updated when TAGGR\u2019s data practices change. The effective date above identifies the current version. Contact the iOS operator privately through the <a href="mailto:qwertyuiop123456@gmail.com">operator email address</a>. Do not post private safety reports to a public issue tracker.</p>
    `
});
var terms = layout({
  ...
  body: `
      ...
      <p>For support or appeals, contact <a href="mailto:qwertyuiop123456@gmail.com">qwertyuiop123456@gmail.com</a>.</p>
