...
extension TaggrTests {
    ...
    @MainActor
    func testReportDoesNotUseCanisterOrRequireBalance() async throws {
        var calls: [(method: String, arg: Data)] = []
        let api = makeStubbedAPI { request in
            if let call = self.requestMethodAndArg(from: request) {
                calls.append(call)
        ...
        let id = UUID()
        try await state.report(userId: 7, postId: 42, reason: "misbehavior", requestID: id)
        XCTAssertTrue(calls.isEmpty)
        let report = try XCTUnwrap(SafetyURLProtocolStub.lastReport)
        XCTAssertEqual(report.id, id.uuidString)
        XCTAssertEqual(report.postID, 42)
        XCTAssertEqual(report.userID, 7)
        XCTAssertEqual(report.reason, "misbehavior")
    }
