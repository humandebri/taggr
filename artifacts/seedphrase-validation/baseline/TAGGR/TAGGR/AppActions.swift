...
extension TaggrAppCoordinator {
    ...
    private func performRepost(
        ...
        do {
            ...
            guard context.runtimeGeneration == runtimeGeneration,
                  ...
                  safetyPolicy?.postIDs.contains(postId) != true else { throw TaggrSafetyError.unavailable }
        } catch {
            return TaggrPostSubmissionResult(outcome: .retryableFailure, errorMessage: error.localizedDescription)
        }
        do {
    ...
    func report(userId: Int, postId: Int? = nil, reason: String, requestID: UUID) async throws {
        try await safety.send(TaggrSafetyReport(id: requestID.uuidString, canisterID: runtimeConfig.canisterId,
            kind: postId == nil ? "user" : "post", postID: postId, userID: userId, reason: reason))
    }
    ...
    func toggleBlock(userId: Int) async {
        ...
        safety.setBlocked(blocked, userID: userId, scope: safetyScope, canisterID: runtimeConfig.canisterId)
        // Persist the local choice first; IC or notification failures must not expose blocked content.
        ...
        await safety.flushBlockNotifications()
    }
    ...
    func completeIdentity(_ session: ICAuthSession) async {
        let completed = await runBusy {
            ...
            authSession = session
            safetyConfirmedUserScope = safetyScope
            await reloadCache()
            await loadCurrentRoute()
        }
        if completed {
