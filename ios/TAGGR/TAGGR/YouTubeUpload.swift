...
final class YouTubeUploadCoordinator {
    ...
    var safetyCheck: (@MainActor (YouTubeDraftTarget, String) async throws -> Void)?
    ...
    func start(
        ...
        do {
            try await safetyCheck?(target, metadata.title + " " + metadata.description)
    ...
    private func reconcileBackgroundTasks() async {
        let tasks = await session.allTasks
        if let job, job.phase != .completed {
            do { try await safetyCheck?(job.target, job.metadata.title + " " + job.metadata.description) }
            catch {
                tasks.forEach { $0.cancel() }
                failPermanently(error.localizedDescription)
                return
            }
        }
    ...
    private func resumeAfterInterruption() async {
        ...
        do {
            try await safetyCheck?(job.target, job.metadata.title + " " + job.metadata.description)
    ...
    private func enqueueNextChunk() async throws {
        ...
        try await safetyCheck?(job.target, job.metadata.title + " " + job.metadata.description)
        guard self.job?.id == job.id else { throw TaggrSafetyError.unavailable }
