...
extension TaggrTests {
    ...
    func testProfileRouteAndPostsLoadEachEndpointOnce() async throws {
        ...
        state.navigateToProfile(try XCTUnwrap(state.currentUser).name)
    ...
    func testProfileTransitionClearsPreviousPostsBeforeTheNextProfileLoads() async throws {
        ...
        let api = makeStubbedAPI { request in
            ...
            if method == "user" {
                userQueries += 1
                if userQueries == 2 {
                    secondProfileStarted.fulfill()
                    guard releaseSecondProfile.wait(timeout: .now() + 5) == .success else {
                        throw URLError(.timedOut)
                    }
                    return (response, Self.queryReply(Self.userFixture(id: 8, name: "bob")))
                }
                return (response, Self.queryReply(Self.userFixture()))
            }
            let postID = userQueries == 1 ? 303 : 302
            let body = Data("[\(String(data: self.postEnvelopeFixture(id: postID), encoding: .utf8)!)]".utf8)
            return (response, Self.queryReply(body))
        }
        let state = makeCoordinator(api: api)
...
extension TaggrTests {
    ...
    func testWebMultiHunkEditPatchesAndMixedDeletionHistory() throws {
        let source0 = "😀 AAA hello middle middle middle middle middle middle middle middle middle middle world ZZZ"
        let expected0 = "😀 AAA hello123 middle middle middle middle middle middle middle middle middle middle earth ZZZ"
        // Generated by the web client’s diff-match-patch 1.0.5.
        let patch0 = "@@ -5,16 +5,19 @@\n AA hello\n+123\n  middle \n@@ -83,13 +83,13 @@\n dle \n-world\n+earth\n  ZZZ\n"
        XCTAssertEqual(try TaggrEditPatch.apply(patch0, to: source0), expected0)
        let source1 = "😀 AAA hello123 middle middle middle middle middle middle middle middle middle middle earth ZZZ"
        let expected1 = "😀 AAA hello middle middle middle middle middle middle middle middle middle middle world ZZZ"
        // Generated by the web client’s diff-match-patch 1.0.5.
        let patch1 = "@@ -9,11 +9,8 @@\n ello\n-123\n  mid\n@@ -80,13 +80,13 @@\n dle \n-earth\n+world\n  ZZZ\n"
        XCTAssertEqual(try TaggrEditPatch.apply(patch1, to: source1), expected1)
        let latest = "iOS edit"
        let post = samplePost(body: latest, files: [:], patches: [
            [.number(1), .string(patch1)],
            [.number(2), .string(TaggrEditPatch.fullReplacement(from: latest, to: expected0))],
        ])
        XCTAssertEqual(try post.deletionVersions(), [source0, expected0, latest])
        XCTAssertThrowsError(try TaggrEditPatch.apply(patch0, to: "unrelated text"))
    }
}
